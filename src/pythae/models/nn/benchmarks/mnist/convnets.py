...
class Encoder_Conv_FCDVAE_MNIST(BaseEncoder):
    ...
    .. note::
        ...
        Please note that this encoder is only suitable for Variational Autoencoder based models
        since it outputs the embeddings and the **log** of the covariance diagonal coefficients
        of the input data under the key `embedding` and `tri_cov`.
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        self.embedding = nn.Linear(1024, args.latent_dim)
        self.tri_cov = nn.Linear(1024, (args.latent_dim*args.latent_dim+args.latent_dim) // 2)
    ...
    def forward(self, x: torch.Tensor, output_layer_levels: List[int] = None):
        ...
        for i in range(max_depth):
            ...
            if i + 1 == self.depth:
                output["embedding"] = self.embedding(out.reshape(x.shape[0], -1))
                output["tri_cov"] = self.tri_cov(out.reshape(x.shape[0], -1))
